...
export class A2aExecutor implements AgentExecutor {
  private readonly activeTasks = new Map<string, { controller: AbortController, contextId: string, cancelled: boolean }>()
  ...
  async execute(requestContext: RequestContext, eventBus: ExecutionEventBus) {
    ...
    try {
      ...
      if (!prompt) {
        eventBus.publish(AgentEvent.task(this.task(taskId, contextId, userMessage, TaskState.TASK_STATE_REJECTED, {
          narraverse: config,
        }, '消息必须包含非空 text Part。')))
        return
      }
      const history = requestContext.task?.history
        .flatMap(message => message.parts.filter(part => part.content?.$case === 'text').map(part => part.content?.value))
      ...
      const result = await this.runtime.stream(requestContext.context.user!.userName, {
        ...
        context: { ...config.context, a2aContextId: contextId, a2aTaskId: taskId, a2aHistory: history },
      }, controller.signal)
      const metadata = {
        ...(requestContext.task?.metadata ?? {}),
        narraverse: { ...config, sessionId: result.sessionId, runId: result.runId },
      }
      eventBus.publish(AgentEvent.task(this.task(
        taskId,
        contextId,
        userMessage,
      ...
      if (!taskPublished) {
        eventBus.publish(AgentEvent.task(this.task(taskId, contextId, userMessage, state, requestContext.task?.metadata, message)))
      } else {
        eventBus.publish(AgentEvent.statusUpdate(this.status(taskId, contextId, state, message, {
  ...
  async cancelTask(taskId: string, eventBus: ExecutionEventBus) {
    ...
    eventBus.publish(AgentEvent.statusUpdate(this.status(
      taskId,
      active?.contextId ?? '',
    ...
    for await (const chunk of stream) {
      ...
      if (chunk.type === 'text-delta') {
        if (pendingText !== undefined) {
          eventBus.publish(AgentEvent.artifactUpdate(this.artifact(
            taskId,
            contextId,
            artifactId,
            pendingText,
      ...
      } else if (chunk.type === 'error' || chunk.type === 'tool-output-error' || chunk.type === 'tool-input-error') {
        failed = true
      }
    }
  ...
  private resolveContext(requestContext: RequestContext): A2aContext {
    ...
    const metadata = requestContext.request.metadata?.narraverse ?? requestContext.userMessage.metadata?.narraverse
    return a2aContext.parse({
      ...(typeof stored === 'object' && stored ? stored : {}),
      ...(typeof metadata === 'object' && metadata ? metadata : {}),
      ...(typeof messageData === 'object' && messageData ? ('narraverse' in messageData ? messageData.narraverse : messageData) : {}),
    })
  }
  ...
  private agentMessage(contextId: string, taskId: string, text: string, data?: Record<string, unknown>): Message {
    const parts: Part[] = [this.textPart(text)]
    if (data) parts.push({ content: { $case: 'data', value: data }, metadata: undefined, filename: '', mediaType: 'application/json' })
    return {
