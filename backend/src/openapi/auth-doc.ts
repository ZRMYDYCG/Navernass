...
export function appendAuthDocs(document: OpenAPIObject) {
  ...
  document.paths['/api/auth/sign-up/email'] = {
    post: {
      ...
      requestBody: jsonBody({ type: 'object', required: ['name', 'email', 'password'], properties: {
        name: { type: 'string', minLength: 1, example: '张三' },
        email: { type: 'string', format: 'email', example: 'writer@example.com' },
        password: { type: 'string', format: 'password', minLength: 8, maxLength: 128 },
      } }),
      responses: { 200: response('注册成功并写入会话 Cookie', user), 400: response('参数错误或账号已存在') },
    },
  }
  document.paths['/api/auth/sign-in/email'] = {
    post: {
      ...
      requestBody: jsonBody({ type: 'object', required: ['email', 'password'], properties: {
        email: { type: 'string', format: 'email', example: 'writer@example.com' },
        password: { type: 'string', format: 'password', minLength: 8, maxLength: 128 },
  ...
  document.paths['/api/auth/get-session'] = {
    get: {
      ...
      responses: { 200: response('当前会话；未登录时返回 null', { type: ['object', 'null'], additionalProperties: true }) },
    },
  }
  document.paths['/api/auth/sign-out'] = {
    post: {
      ...
      security: [{ 'better-auth': [] }],
      responses: { 200: response('退出成功并清理会话 Cookie'), 401: response('未登录或会话已失效') },
    },
  }
  return document
}
