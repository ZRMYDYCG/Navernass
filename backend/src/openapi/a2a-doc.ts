          ...
          properties: {
            message: {
              ...
              properties: {
                ...
                metadata: {
                  type: 'object',
                  description: '传入 novelId、chapterId、sessionId、providerId、role 等小说上下文。',
  ...
  const paths: Record<string, PathItemObject> = {
    ...
    '/api/v1/a2a/v1/message:send': {
      post: {
        ...
        responses: { 201: a2aResponse('返回 Message 或 Task'), 401: a2aResponse('Bearer Token 无效') },
      },
    },
    '/api/v1/a2a/v1/message:stream': {
      post: {
        ...
        parameters: [versionHeader],
        requestBody: messageBody,
        responses: { 200: { description: 'A2A SSE 事件流', content: { 'text/event-stream': { schema: { type: 'string' } } } }, 401: a2aResponse('Bearer Token 无效') },
      },
    },
    '/api/v1/a2a/v1/tasks/{taskId}': {
      get: {
        ...
        responses: { 200: a2aResponse('Task 当前状态、历史和产物'), 404: a2aResponse('Task 不存在') },
      },
    },
    ...
    '/api/v1/a2a/v1/tasks/{taskId}:subscribe': {
      get: {
        ...
        security,
        parameters: [versionHeader, taskId],
        responses: { 200: { description: 'A2A SSE 事件流', content: { 'text/event-stream': { schema: { type: 'string' } } } } },
      },
    },
  }
