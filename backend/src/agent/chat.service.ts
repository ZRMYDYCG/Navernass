...
export class ChatService {
  ...
  async promptHistory(userId: string, sessionId: string, limit = 20) {
    ...
    return messages.reverse().map(message => `${this.roleLabel(message.role)}：${message.content}`).join('\n\n')
  }
  ...
  private async save(role: 'user' | 'assistant', input: MessageInput) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.agentSession.update({ where: { id: input.sessionId }, data: { updated_at: new Date() } })
      return message
    })
  }

  private async requireSession(userId: string, sessionId: string) {
    const session = await this.prisma.agentSession.findFirst({ where: { id: sessionId, user_id: userId } })
  ...
  private decodeCursor(cursor: string): MessageCursor {
    try {
      ...
      if (!value.createdAt || !value.id || Number.isNaN(new Date(value.createdAt).getTime())) throw new Error('invalid')
