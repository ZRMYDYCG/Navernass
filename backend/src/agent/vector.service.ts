...
export class VectorService {
  ...
  async search(vector: number[], payload: VectorPayload, kinds: string[] | undefined, limit: number, minScore: number) {
    const collection = await this.ensureCollection(vector.length)
  ...
  private async setupCollection(dimension: number) {
    ...
    if (!exists.exists) {
      ...
      await Promise.all([
        this.client.createPayloadIndex(collection, { field_name: 'userId', field_schema: 'keyword', wait: true }),
        this.client.createPayloadIndex(collection, { field_name: 'novelId', field_schema: 'keyword', wait: true }),
        this.client.createPayloadIndex(collection, { field_name: 'chapterId', field_schema: 'keyword', wait: true }),
        this.client.createPayloadIndex(collection, { field_name: 'kind', field_schema: 'keyword', wait: true }),
      ])
    }
    return collection
  }
