...
export class ContentController {
  ...
  @Get('news')
  @AllowAnonymous()
  @ApiDoc({ summary: '分页查询新闻公告', type: ResourceResult, array: true, paged: true, public: true })
  @ApiPageQuery()
  @ApiQuery({ name: 'type', required: false, enum: ['feature', 'update', 'announcement', 'community'] })
  ...
  @Post('surveys')
  @OptionalAuth()
  @ApiDoc({ summary: '提交用户调研', description: '支持匿名提交；登录后会关联当前用户。', type: ResourceResult, status: 201, public: true })
  ...
  @ApiQuery({ name: 'id', required: true, type: String, format: 'uuid', description: '待办事项 UUID' })
  ...
  @Get('message-wall')
  @AllowAnonymous()
  @ApiDoc({ summary: '分页查询留言墙', type: ResourceResult, array: true, paged: true, public: true })
  @ApiPageQuery()
