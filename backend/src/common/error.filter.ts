...
function normalizeError(exception: unknown) {
  ...
  if (exception instanceof Prisma.PrismaClientKnownRequestError) {
    if (exception.code === 'P2002') {
      return { status: 409, code: 'CONFLICT', message: '数据已存在', details: safeMeta(exception.meta) }
    }
  ...
  return {
    ...
    code: 'INTERNAL_ERROR',
    message: process.env.NODE_ENV === 'production' ? '服务器内部错误' : String((exception as Error)?.message ?? exception),
  }
}
