...
export class MessagesService {
  ...
  async create(messageData: CreateMessageDto) {
    ...
    try {
      ...
      if (isMissingPartsColumn) {
        console.warn(
          '[messages] parts 列不存在，自动降级（请尽快执行迁移 add_messages_parts.sql）',
        )
   ...
   *
   * 关键兼容性：ai-sdk v6 的 useChat 自动生成的 message id 是短随机字符串
   * （如 "poFNmEqMoOSggAbV"），不是 uuid。messages.id 列是 uuid 类型，直接
   * upsert 会触发 22P02 invalid input syntax for type uuid。
   *
   * 处理：
   ...
   */
  async upsert(message: { id: string } & CreateMessageDto) {
    const { data: { user } } = await this.supabase.auth.getUser()
    ...
    try {
      ...
      if (isMissingPartsColumn) {
        ...
        console.error('[messages] upsert failed:', err)
        throw err
      }
    }
   ...
   * 更新消息
   */
  async update(id: string, updates: Partial<Pick<Message, 'content' | 'parts' | 'thinking'>>) {
    const buildPayload = (includeParts: boolean) => {
