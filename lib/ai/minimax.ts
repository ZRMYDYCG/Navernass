 ...
 *
 * 当前 provider：MiniMax（M2 系列模型走 OpenAI 兼容协议）。
 ...
 * - M2.5 / M2.7：开头与结尾都发完整 <think>…</think> 标签 → startWithReasoning: false
 */
...
export const DEFAULT_LLM_MODEL = process.env.LLM_MODEL || 'MiniMax-M2.7'
...
/** 模型 id 是否属于 MiniMax M2 系列（需要 think 标签提取） */
function isM2Series(modelId: string): boolean {
  return /^MiniMax-M2(\.|$)/i.test(modelId)
}

/**
 * 流的起始是否处于 reasoning 中（无前置 <think>）。
 * 仅老 M2 是这种模式；M2.5/M2.7 都会显式发 <think>。
 */
