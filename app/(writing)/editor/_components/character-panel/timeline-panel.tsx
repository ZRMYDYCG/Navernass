...
export function TimelinePanel({ novelId, character, onClose, onOpenScriptChat }: TimelinePanelProps) {
  ...
  const hydratedCharacters = useTimelineStore(s => s.hydratedCharacters)
  const hydrate = useTimelineStore(s => s.hydrateForCharacter)
  const upsert = useTimelineStore(s => s.upsertEvent)
  const remove = useTimelineStore(s => s.removeEvent)
