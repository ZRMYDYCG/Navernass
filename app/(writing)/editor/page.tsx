...
import { selectOrderedChapters, selectOrderedVolumes, useChaptersStore, usePlanStore, useCharacterMaterialStore, useCharacterGraphStore, useAiEditsStore } from '@/store'
...
function NovelsEditContent() {
  ...
  const setChapters = useChaptersStore(s => s.chaptersActions.setChapters)
  const setVolumes = useChaptersStore(s => s.chaptersActions.setVolumes)
  // eslint-disable-next-line unused-imports/no-unused-vars
  const [characters, setCharacters] = useState<NovelCharacter[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedChapter, setSelectedChapter] = useState<string | null>(null)
  const selectedPlanFileId = usePlanStore(s => s.plan.selectedPlanFileId)
  const setSelectedPlanFileId = usePlanStore(s => s.planActions.setSelectedPlanFileId)
  ...
  const loadData = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      useChaptersStore.getState().chaptersActions.hydrate(novelId, chaptersData, volumesData)
      usePlanStore.getState().planActions.resetForNovel(novelId)

      // 存入角色素材 store
      useCharacterMaterialStore.getState().characterMaterialActions.setCharacters(charactersData)
      // 存入关系图 store，保证刷新后连线存在
      useCharacterGraphStore.getState().characterGraphActions.setRelationships(novelId, relationshipsData)
    } catch (error) {
  ...
  const handleSelectChapter = (chapterId: string) => {
    ...
    const previewChapterId = useCharacterGraphStore.getState().characterGraph.chapterCharacterPreviewChapterId
    if (previewChapterId && previewChapterId !== chapterId) {
      useCharacterGraphStore.getState().characterGraphActions.setChapterCharacterPreview(null)
    }
  ...
  const handleSelectPlanFile = (planFileId: string) => {
    const file = usePlanStore.getState().plan.planFilesById[planFileId]
    if (!file) return
  ...
  const focusEdit = useAiEditsStore(s => (focusEditId ? s.aiEdits.edits[focusEditId] : undefined))
  ...
  const handleConfirmDeleteChapter = async () => {
    ...
    try {
      ...
      useChaptersStore.getState().chaptersActions.removeChapter(chapterToDelete.id)
