  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // 根据模式构建系统提示词
        let systemPrompt = '你是一个专业的小说创作助手，擅长帮助用户构思情节、塑造角色、续写故事。请用温暖、鼓励的语气与用户交流，提供有创意的建议。'
        if (mode === 'agent') {
          systemPrompt = '你是一个智能的小说创作助手，可以主动分析用户的小说内容，提供创作建议、情节规划、角色塑造等多方面的帮助。'
        } else if (mode === 'plan') {
          systemPrompt = '你是一个专业的小说规划助手，擅长帮助用户制定创作计划、梳理故事结构、规划章节内容。'
        }
