...
export class PlanningController {
  ...
  @ApiQuery({
    name: "category",
    required: false,
    enum: ["setting", "location", "item", "faction", "event", "rule", "character_lore", "other"],
  })
  listWorldbook(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Query(new ZodPipe(s.novelQuery)) q: z.infer<typeof s.novelQuery>,
  ) {
  ...
  createWorldbook(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.createWorldbook.omit({ novel_id: true }))) body: Omit<
      s.CreateWorldbook,
      "novel_id"
    >,
  ) {
  ...
  updateWorldbook(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.updateWorldbook)) body: s.UpdateWorldbook,
  ) {
  ...
  createOutline(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.createOutline.omit({ novel_id: true }))) body: Omit<
      s.CreateOutline,
      "novel_id"
    >,
  ) {
  ...
  updateOutline(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.updateOutline)) body: s.UpdateOutline,
  ) {
  ...
  createPlan(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.createPlan.omit({ novel_id: true }))) body: Omit<s.CreatePlan, "novel_id">,
  ) {
  ...
  updatePlan(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.updatePlan)) body: s.UpdatePlan,
  ) {
  ...
  createTimeline(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.createTimeline.omit({ novel_id: true }))) body: Omit<
      s.CreateTimeline,
      "novel_id"
    >,
  ) {
  ...
  updateTimeline(
    @CurrentUser() user: AuthUser,
    @Param(new ZodPipe(idParam)) p: { id: string },
    @Body(new ZodPipe(s.updateTimeline)) body: s.UpdateTimeline,
  ) {
