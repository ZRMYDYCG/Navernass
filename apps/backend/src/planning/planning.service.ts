...
export class PlanningService {
  ...
  async listWorldbook(
    userId: string,
    novelId: string,
    category?: input.CreateWorldbook["category"],
  ) {
  ...
  async createWorldbook(userId: string, data: input.CreateWorldbook) {
    ...
    return this.prisma.worldbookEntry.create({
      data: { ...data, user_id: userId, keywords: data.keywords },
    });
  }
  ...
  async listOutlines(
    userId: string,
    novelId: string,
    query: { volumeId?: string; parentId?: string | null },
  ) {
    await this.ownNovel(userId, novelId);
  ...
  async createPlan(userId: string, data: input.CreatePlan) {
    await this.ownNovel(userId, data.novel_id);
    const exists = await this.prisma.planFile.findFirst({
      where: { novel_id: data.novel_id, path: data.path, deleted_at: null },
    });
  ...
  async updatePlan(userId: string, id: string, data: input.UpdatePlan) {
    ...
    if (data.path && data.path !== current.path) {
      const exists = await this.prisma.planFile.findFirst({
        where: { novel_id: current.novel_id, path: data.path, NOT: { id }, deleted_at: null },
      });
      if (exists) throw new AppError("PATH_EXISTS", "规划文件路径已存在", 409);
    }
  ...
  async createTimeline(userId: string, data: input.CreateTimeline) {
    ...
    const characters = Array.isArray(novel.characters)
      ? (novel.characters as Array<{ id?: string }>)
      : [];
    if (!characters.some((item) => item.id === data.character_id))
      throw AppError.notFound("CHARACTER_NOT_FOUND", "角色");
    if (data.chapter_id) {
      const chapter = await this.prisma.chapter.findFirst({
        where: { id: data.chapter_id, user_id: userId, novel_id: data.novel_id },
      });
    ...
    return this.prisma.timelineEvent.create({
      data: {
        ...
        occurred_at_label: data.occurred_at_label ?? null,
      },
    });
  }
  ...
  async updateTimeline(userId: string, id: string, data: input.UpdateTimeline) {
    ...
    if (data.chapter_id) {
      const chapter = await this.prisma.chapter.findFirst({
        where: { id: data.chapter_id, user_id: userId, novel_id: current.novel_id },
      });
      if (!chapter) throw AppError.notFound("CHAPTER_NOT_FOUND", "章节");
    }
  ...
  private async ownRecord(
    type: "worldbook" | "outline" | "plan" | "timeline",
    userId: string,
    id: string,
  ): Promise<any> {
  ...
  private async validateOutlineLinks(
    userId: string,
    novelId: string,
    ...
    if (volumeId) {
      const volume = await this.prisma.volume.findFirst({
        where: { id: volumeId, novel_id: novelId, user_id: userId, deleted_at: null },
      });
      if (!volume) throw AppError.notFound("VOLUME_NOT_FOUND", "卷");
    }
    if (parentId) {
      const parent = await this.prisma.outline.findFirst({
        where: { id: parentId, novel_id: novelId, user_id: userId, deleted_at: null },
      });
  ...
  private async isDescendant(ancestorId: string, candidateId: string) {
    ...
    for (let depth = 0; depth < 100 && cursor; depth += 1) {
      ...
      const node: { parent_id: string | null } | null = await this.prisma.outline.findUnique({
        where: { id: cursor },
