...
export const updateChapter = createChapter
  .omit({ novel_id: true })
  .partial()
  .extend({
...
export const updateRelationship = relationshipFields
  .omit({ novel_id: true })
  .partial()
  .refine((value) => !value.sourceId || !value.targetId || value.sourceId !== value.targetId, {
    message: "关系双方不能是同一个角色",
    path: ["targetId"],
  });
