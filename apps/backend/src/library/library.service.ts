...
export class LibraryService {
  ...
  async listNovels(userId: string, query: { page: number, pageSize: number, status?: 'draft' | 'published' | 'archived' }) {
  ...
  async createNovel(userId: string, input: CreateNovelInput) {
    const max = await this.prisma.novel.aggregate({ where: { user_id: userId }, _max: { order_index: true } })
  ...
  async setNovelStatus(userId: string, id: string, status: 'draft' | 'published' | 'archived') {
    ...
    return this.prisma.$transaction(async tx => {
      if (status === 'archived') {
        const now = new Date()
        await Promise.all([
          tx.chapter.updateMany({ where: { novel_id: id, user_id: userId }, data: { deleted_at: now } }),
          tx.volume.updateMany({ where: { novel_id: id, user_id: userId }, data: { deleted_at: now } }),
        ])
      }
      if (status === 'draft' && novel.status === 'archived') {
        await Promise.all([
          tx.chapter.updateMany({ where: { novel_id: id, user_id: userId }, data: { deleted_at: null } }),
          tx.volume.updateMany({ where: { novel_id: id, user_id: userId }, data: { deleted_at: null } }),
        ])
      }
      return tx.novel.update({
        where: { id },
        data: { status, published_at: status === 'published' ? new Date() : status === 'draft' ? null : undefined },
      })
    })
  }

  async reorderNovels(userId: string, items: Array<{ id: string, order_index: number }>) {
    await this.ensureOwnedIds('novel', userId, items.map(item => item.id))
    await this.prisma.$transaction(items.map(item => this.prisma.novel.update({
      where: { id: item.id }, data: { order_index: item.order_index },
    })))
  }
  ...
  async getVolume(userId: string, id: string) {
    const volume = await this.prisma.volume.findFirst({ where: { id, user_id: userId, deleted_at: null } })
  ...
  async reorderVolumes(userId: string, items: Array<{ id: string, order_index: number }>) {
    await this.ensureOwnedIds('volume', userId, items.map(item => item.id))
    await this.prisma.$transaction(items.map(item => this.prisma.volume.update({ where: { id: item.id }, data: { order_index: item.order_index } })))
  }

  async listChapters(userId: string, novelId: string, volumeId?: string) {
    await this.getNovel(userId, novelId)
    return this.prisma.chapter.findMany({
      where: { novel_id: novelId, user_id: userId, deleted_at: null, ...(volumeId && { volume_id: volumeId }) },
      orderBy: { order_index: 'asc' },
    })
  }

  async getChapter(userId: string, id: string) {
    const chapter = await this.prisma.chapter.findFirst({ where: { id, user_id: userId, deleted_at: null } })
  ...
  async createChapter(userId: string, input: CreateChapterInput) {
    ...
    return this.prisma.$transaction(async tx => {
      const chapter = await tx.chapter.create({ data: {
        ...input,
        user_id: userId,
        volume_id: input.volume_id ?? null,
        word_count: countWords(input.content),
      } })
      await tx.novel.update({ where: { id: input.novel_id }, data: {
  ...
  async updateChapter(userId: string, id: string, input: UpdateChapterInput) {
    ...
    if (input.volume_id) {
      const volume = await this.getVolume(userId, input.volume_id)
      if (volume.novel_id !== existing.novel_id) throw new AppError('BAD_REQUEST', '卷与章节不属于同一本小说')
    }
    const nextCount = input.content === undefined ? existing.word_count : countWords(input.content)
    return this.prisma.$transaction(async tx => {
      const chapter = await tx.chapter.update({ where: { id }, data: { ...input, word_count: nextCount } })
      const delta = nextCount - existing.word_count
      if (delta !== 0) await tx.novel.update({ where: { id: existing.novel_id }, data: { word_count: { increment: delta } } })
      return chapter
    })
  }
  ...
  async deleteChapter(userId: string, id: string) {
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.novel.update({ where: { id: chapter.novel_id }, data: {
  ...
  async publishChapter(userId: string, id: string, published: boolean) {
    ...
    return this.prisma.chapter.update({ where: { id }, data: { status: published ? 'published' : 'draft' } })
  }

  async reorderChapters(userId: string, items: Array<{ id: string, order_index: number }>) {
    await this.ensureOwnedIds('chapter', userId, items.map(item => item.id))
    await this.prisma.$transaction(items.map(item => this.prisma.chapter.update({ where: { id: item.id }, data: { order_index: item.order_index } })))
  }

  async searchChapters(userId: string, input: { novelId: string, keyword: string, volumeId?: string | null, excludeVolumeId?: string | null }) {
  ...
  async publishedNovel(id: string) {
    ...
    const [volumes, chapters] = await Promise.all([
      this.prisma.volume.findMany({ where: { novel_id: id, deleted_at: null }, orderBy: { order_index: 'asc' } }),
      this.prisma.chapter.findMany({ where: { novel_id: id, deleted_at: null, status: 'published' }, orderBy: { order_index: 'asc' } }),
    ])
  ...
  async createCharacter(userId: string, input: CreateCharacterInput) {
    ...
    const character: Character = { ...fields, id: crypto.randomUUID(), order_index: input.order_index ?? list.length }
    await this.prisma.novel.update({ where: { id: novel.id }, data: { characters: [...list, character] } })
    return character
  }
  ...
  async deleteCharacter(userId: string, id: string) {
    ...
    const relationships = located.relationships.filter(item => item.sourceId !== id && item.targetId !== id)
    await this.prisma.$transaction([
      this.prisma.novel.update({ where: { id: located.novelId }, data: { characters, relationships } }),
      this.prisma.timelineEvent.updateMany({ where: { novel_id: located.novelId, character_id: id }, data: { deleted_at: new Date() } }),
    ])
  }
  ...
  async createRelationship(userId: string, input: CreateRelationshipInput) {
    ...
    if (!characters.some(item => item.id === input.sourceId) || !characters.some(item => item.id === input.targetId)) {
    ...
    await this.prisma.novel.update({ where: { id: novel.id }, data: { relationships: [...list, relationship] } })
    return relationship
  }
  ...
  async updateRelationship(userId: string, id: string, input: UpdateRelationshipInput) {
    ...
    await this.prisma.novel.update({
      ...
      data: { relationships: located.relationships.map(item => item.id === id ? updated : item) },
    })
    return updated
  }
  ...
  private async findCharacter(userId: string, id: string) {
    const novels = await this.prisma.novel.findMany({ where: { user_id: userId }, select: { id: true, characters: true, relationships: true } })
    for (const novel of novels) {
      const characters = jsonList<Character>(novel.characters)
      const character = characters.find(item => item.id === id)
      if (character) return { novelId: novel.id, character, characters, relationships: jsonList<Relationship>(novel.relationships) }
    }
    throw AppError.notFound('CHARACTER_NOT_FOUND', '角色')
  }

  private async findRelationship(userId: string, id: string) {
    const novels = await this.prisma.novel.findMany({ where: { user_id: userId }, select: { id: true, relationships: true } })
    for (const novel of novels) {
  ...
  private async ensureOwnedIds(model: 'novel' | 'volume' | 'chapter', userId: string, ids: string[]) {
    const count = model === 'novel'
