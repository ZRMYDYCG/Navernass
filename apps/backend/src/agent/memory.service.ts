...
export class MemoryService {
  ...
  async search(userId: string, input: SearchMemory) {
    ...
    const vectorScores = new Map<string, number>(
      vectorHits.map((hit) => [String(hit.id), hit.score]),
    );
    ...
    const vectorRows = vectorScores.size
      ? await this.prisma.semanticMemory.findMany({
          where: { vector_id: { in: [...vectorScores.keys()] } },
        })
      : [];
    const merged = new Map<
      string,
      { item: (typeof keywordHits)[number]; score: number; sources: string[] }
    >();
    for (const item of vectorRows) {
      merged.set(item.id, {
        item,
        score: vectorScores.get(item.vector_id) ?? 0,
        sources: ["vector"],
      });
    }
  ...
  async sync(userId: string, input: SyncMemory) {
    ...
    if (input.kinds.includes("worldbook")) {
      const rows = await this.prisma.worldbookEntry.findMany({
        where: { novel_id: input.novelId, user_id: userId, deleted_at: null },
      });
      sources.push(
        ...rows.map((row) => ({
    ...
    if (input.kinds.includes("outline")) {
      const rows = await this.prisma.outline.findMany({
        where: { novel_id: input.novelId, user_id: userId, deleted_at: null },
      });
      sources.push(
        ...rows.map((row) => ({
    ...
    if (input.kinds.includes("timeline")) {
      ...
      sources.push(
        ...rows.map((row) => ({
          ...
          metadata: { characterId: row.character_id, position: row.timeline_position },
          providerId: input.providerId,
        })),
      );
    }
    ...
    for (const source of sources) {
      ...
      const current = source.sourceId
        ? await this.prisma.semanticMemory.findFirst({
            where: { novel_id: input.novelId, kind: source.kind, source_id: source.sourceId },
          })
        : null;
