  ...
  build(context: ToolContext, mode: RunAgent['mode'] = 'agent'): ToolSet {
    ...
    const available: ToolSet = {
      ...
      getNovelSnapshot: tool({
        ...
        execute: (input, options) => observed(options.toolCallId, 'getNovelSnapshot', input, async () => {
          const novel = await this.prisma.novel.findFirstOrThrow({
            ...
            include: {
              ...
              timeline_events: { where: { deleted_at: null }, orderBy: { timeline_position: 'asc' } },
            },
          })
          return {
            ...novel,
      ...
      getChapter: tool({
        ...
        execute: (input, options) => observed(options.toolCallId, 'getChapter', input, () => this.prisma.chapter.findFirstOrThrow({
          where: { id: input.chapterId, novel_id: context.input.novelId, user_id: context.userId, deleted_at: null },
        })),
      }),
      searchMemory: tool({
        ...
        inputSchema: z.object({
          ...
          kinds: z.array(z.enum(['chapter', 'character', 'worldbook', 'outline', 'timeline', 'summary', 'conversation', 'custom'])).optional(),
          limit: z.number().int().min(1).max(20).default(8),
        }),
        execute: (input, options) => observed(options.toolCallId, 'searchMemory', input, () => this.memories.search(context.userId, {
      ...
      saveMemory: tool({
        ...
        execute: (input, options) => observed(options.toolCallId, 'saveMemory', input, () => this.memories.save(context.userId, {
          novelId: context.input.novelId,
          chapterId: context.input.chapterId,
          providerId: context.input.providerId,
          kind: input.kind,
      ...
      validateContinuity: tool({
        ...
        inputSchema: z.object({ text: z.string().min(1).max(100_000), focus: z.string().max(500).optional() }),
        execute: (input, options) => observed(options.toolCallId, 'validateContinuity', input, async () => {
      ...
      delegateSubagent: tool({
        ...
        execute: (input, options) => observed(options.toolCallId, 'delegateSubagent', input, async () => {
          const result = await generateText({
            model: context.model,
