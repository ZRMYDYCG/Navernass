...
export class ModelService {
  ...
  async language(userId: string, providerId?: string): Promise<{
    model: LanguageModel
    provider: Awaited<ReturnType<ProviderService['resolve']>>
  }> {
    const provider = await this.providers.resolve(userId, providerId)
    const options = { apiKey: provider.apiKey, ...(provider.base_url && { baseURL: provider.base_url }) }
    switch (provider.kind) {
      ...
      default: {
        const baseURL = provider.base_url ?? compatibleBaseUrls[provider.kind as keyof typeof compatibleBaseUrls]
        if (!baseURL) {
          throw new AppError('AI_PROVIDER_INVALID', '兼容 Provider 必须配置 baseUrl', HttpStatus.BAD_REQUEST)
        }
  ...
  async embedding(userId: string, providerId?: string): Promise<{
    model: EmbeddingModel
    provider: Awaited<ReturnType<ProviderService['resolve']>>
  }> {
    const provider = await this.providers.resolve(userId, providerId)
    if (!provider.embedding_model) {
      throw new AppError('EMBEDDING_NOT_CONFIGURED', '当前 Provider 未配置 embeddingModel', HttpStatus.BAD_REQUEST)
    }
    const options = { apiKey: provider.apiKey, ...(provider.base_url && { baseURL: provider.base_url }) }
    ...
    if (provider.kind === 'google') {
      return { provider, model: createGoogleGenerativeAI(options).embeddingModel(provider.embedding_model) }
    }
    if (provider.kind === 'anthropic') {
      throw new AppError('EMBEDDING_NOT_SUPPORTED', 'Anthropic 不提供嵌入模型，请选择 OpenAI 或兼容 Provider', HttpStatus.BAD_REQUEST)
    }
    const baseURL = provider.base_url ?? compatibleBaseUrls[provider.kind as keyof typeof compatibleBaseUrls]
    if (!baseURL) {
      throw new AppError('AI_PROVIDER_INVALID', '兼容 Provider 必须配置 baseUrl', HttpStatus.BAD_REQUEST)
