...
export class SkillService {
    ...
    const [rows, total] = await this.prisma.$transaction([
      this.prisma.skillDef.findMany({
        ...
        include: {
          ...
          novels: query.novelId
            ? { where: { user_id: userId, novel_id: query.novelId }, take: 1 }
            : false,
        },
      }),
  ...
  async listCustom(userId: string) {
    return this.prisma.skillDef
      ...
      .then((rows) =>
        rows.map((row) => ({ ...this.toItem(row), skillMd: row.skill_md, manifest: row.manifest })),
      );
  }
  ...
  async bindNovel(userId: string, novelId: string, input: BindNovelSkills) {
    ...
    if (ids.length) {
      ...
      const allowed = new Set(
        accessible
          .filter(
            (skill) => skill.is_builtin || skill.owner_id === userId || skill.installs.length > 0,
          )
          .map((skill) => skill.id),
      );
      const missing = ids.filter((id) => !allowed.has(id));
      if (missing.length)
        throw new AppError("SKILL_NOT_FOUND", `Skill 未安装或不存在：${missing.join(", ")}`, 404);
    }
    await this.prisma.$transaction(async (tx) => {
      await tx.novelSkill.deleteMany({ where: { user_id: userId, novel_id: novelId } });
      if (input.skills.length) {
        await tx.novelSkill.createMany({
  ...
  private async requireCustom(userId: string, id: string) {
    const skill = await this.prisma.skillDef.findFirst({
      where: { id, owner_id: userId, source: "custom" },
    });
