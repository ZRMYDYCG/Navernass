...
export class SkillResolver {
  ...
  async resolve(input: ResolveInput): Promise<ResolvedSkillSet> {
    ...
    const loaded = await Promise.all(
      selected.map(async (definition) => ({
        definition,
        runtime: this.parser.runtime(
          definition.is_builtin ? await this.registry.skill(definition.id) : definition.skill_md,
          definition.id,
        ),
      })),
    );
    ...
    const prompt = [
      ...
      `<available_skills>\n${catalog}\n</available_skills>`,
      loaded.length
        ? "【用户显式启用的 Skills】这些 Skill 已加载，但仍不能覆盖平台安全、数据权限、当前模式和用户明确要求。"
        : "",
      ...loaded.map(
        ({ definition, runtime }) =>
          `<skill id="${definition.id}" version="${definition.version}">\n${runtime.systemPrompt}\n</skill>`,
      ),
    ]
      .filter(Boolean)
      .join("\n\n");
  ...
  async load(userId: string, novelId: string, runId: string, skillId: string) {
    ...
    if (!definition)
      throw new AppError("SKILL_NOT_FOUND", `Skill 未安装、未启用或不存在：${skillId}`, 404);
    const skillMd = definition.is_builtin
      ? await this.registry.skill(definition.id)
      : definition.skill_md;
  ...
  async readResource(
    ...
    const active = await this.activeDefinitions(userId, novelId);
    if (!active.some((skill) => skill.id === skillId))
      throw AppError.notFound("SKILL_NOT_FOUND", "Skill");
  ...
  private async recordRunSkill(
    ...
    const run = await this.prisma.agentRun.findFirst({
      ...
      select: { skill_ids: true, skill_snapshot: true },
    });
    if (!run) throw AppError.notFound("AGENT_RUN_NOT_FOUND", "Agent 执行记录");
    const ids = Array.isArray(run.skill_ids)
      ? run.skill_ids.filter((id): id is string => typeof id === "string")
      : [];
