...
export class SkillController {
  ...
  @Get("skills")
  @ApiDoc({
    summary: "分页查询 Skill 市场",
    description: "返回内置、社区及当前用户自定义 Skill，并合并安装和小说覆盖状态。",
    type: ResourceResult,
    array: true,
    paged: true,
  })
  ...
  async list(
    ...
    @Query(new ZodPipe(schema.skillQuery)) query: schema.SkillQuery,
  ) {
  ...
  @Post("skills/custom")
  @ApiDoc({
    summary: "创建并校验自定义 SKILL.md",
    description: "只接受声明式 Prompt 和 Tool 名称，不执行任意代码。",
    type: ResourceResult,
    status: 201,
  })
  ...
  createCustom(
    @CurrentUser() user: AuthUser,
    @Body(new ZodPipe(schema.customSkill)) body: schema.CustomSkill,
  ) {
