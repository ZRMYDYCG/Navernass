...
export class AdminController {
  ...
  @ApiParam({ name: 'resource', enum: resourceParam.shape.resource.options, description: '资源类型' })
  @ApiPageQuery()
  ...
  @ApiParam({ name: 'resource', enum: resourceParam.shape.resource.options, description: '资源类型' })
  @ApiQuery({ name: 'id', required: true, type: String, format: 'uuid', description: '待删除资源 UUID' })
  async delete(
