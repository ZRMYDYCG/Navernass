...
export class AdminService {
  ...
  async list(resource: AdminResource, page: number, pageSize: number, status?: string) {
    ...
    switch (resource) {
      ...
      case "profiles":
        return this.page(
          this.prisma.profile.findMany({ ...pageArgs, orderBy: { created_at: "desc" } }),
          this.prisma.profile.count(),
        );
      case "novels":
        return this.page(
          ...
          this.prisma.novel.count({ where: status ? { status: status as any } : {} }),
        );
      case "chapters":
        return this.page(
          this.prisma.chapter.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.chapter.count(),
        );
      case "volumes":
        return this.page(
          this.prisma.volume.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.volume.count(),
        );
      case "surveys":
        return this.page(
          this.prisma.survey.findMany({ ...pageArgs, orderBy: { created_at: "desc" } }),
          this.prisma.survey.count(),
        );
      case "news":
        return this.page(
          this.prisma.news.findMany({ ...pageArgs, orderBy: { created_at: "desc" } }),
          this.prisma.news.count(),
        );
      case "message-wall":
        return this.page(
          this.prisma.messageWallEntry.findMany({ ...pageArgs, orderBy: { created_at: "desc" } }),
          this.prisma.messageWallEntry.count(),
        );
      case "writer-todos":
        return this.page(
          this.prisma.writerTodo.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.writerTodo.count(),
        );
      case "plan-files":
        return this.page(
          this.prisma.planFile.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.planFile.count(),
        );
      case "worldbook":
        return this.page(
          this.prisma.worldbookEntry.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.worldbookEntry.count(),
        );
      case "outlines":
        return this.page(
          this.prisma.outline.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.outline.count(),
        );
      case "timeline-events":
        return this.page(
          this.prisma.timelineEvent.findMany({ ...pageArgs, orderBy: { updated_at: "desc" } }),
          this.prisma.timelineEvent.count(),
        );
    }
  }
  ...
  async delete(resource: AdminResource, id: string, actorId: string) {
    if (resource === "users") {
      ...
      if (profile?.is_protected || profile?.role === "super_admin")
        throw new AppError("PROTECTED_ADMIN", "不能删除受保护的超级管理员", 409);
    ...
    if (resource === "message-wall")
      return void (await this.prisma.messageWallEntry.delete({ where: { id } }));
    if (resource === "writer-todos")
      return void (await this.prisma.writerTodo.delete({ where: { id } }));
