...
export class A2aTaskStore implements TaskStore {
  ...
  async list(params: ListTasksRequest, context: ServerCallContext): Promise<ListTasksResponse> {
    ...
    const baseWhere: Prisma.A2aTaskWhereInput = {
      ...
      ...(params.statusTimestampAfter && { updated_at: { gte: new Date(params.statusTimestampAfter) } }),
    }
    ...
    return {
      tasks,
      nextPageToken: hasMore && last ? this.encodeCursor({ updatedAt: last.updated_at.toISOString(), id: last.id }) : '',
      pageSize,
      totalSize,
    }
  }
  ...
  private userId(context: ServerCallContext) {
    ...
    if (!user?.isAuthenticated || !user.userName) throw new RequestMalformedError('A2A 请求缺少有效身份。')
  ...
  private decodeCursor(value: string): PageCursor {
    try {
      const cursor = JSON.parse(Buffer.from(value, 'base64url').toString('utf8')) as Partial<PageCursor>
      if (!cursor.updatedAt || !cursor.id || Number.isNaN(new Date(cursor.updatedAt).getTime())) throw new Error('invalid cursor')
