...
export class A2aGateway {
  ...
  constructor(
    ...
    const userBuilder = async (
      request: Parameters<Parameters<typeof restHandler>[0]["userBuilder"]>[0],
    ) => {
  ...
  private agentCard(baseUrl: string, documentationUrl: string): AgentCard {
    ...
    return {
      ...
      description:
        "具备结构化小说记忆、语义召回、工具循环和专业 Subagent 协作能力的小说创作 Agent。",
      supportedInterfaces: [
        { url: baseUrl, protocolBinding: "HTTP+JSON", protocolVersion: "1.0", tenant: "" },
      ],
      ...
      skills: [
        this.skill(
          "novel-creation",
          "小说协同创作",
          "统筹上下文、记忆、工具与 Subagent 完成小说创作任务。",
          ["小说", "创作", "agent"],
          securityRequirements,
        ),
        this.skill(
  ...
  private skill(
    ...
    description: string,
    tags: string[],
    securityRequirements: AgentCard["securityRequirements"],
  ) {
    return {
      id,
      name,
